...
const MergePause = () => {
    ...
    function merge(){
        if (fs.existsSync(path.join(app.getPath('userData'), 'recordings', book.bookName, chapter, `temp.mp3`))){
            ...
            existingVerse = (path.join(app.getPath('userData'), 'recordings',book.bookName, chapter , `verse${onselect}.mp3`))
            ...
            audio
                    ...
                    .then(() => {
                        ...
                        writeRecfile(output, path.join(app.getPath('userData'), 'recordings',book.bookName, chapter , `verse${onselect}.mp3`))
                        // => {blob, element, url}
                        // audio.download(
                        // 	output.blob,
