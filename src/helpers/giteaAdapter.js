...
export default class Gitea {
    ...
    */
    ...
    async getProjects(attempt = 1) {
        ...
        //     }
        // }
        let response = await axios.get(`https://${this.endpoint}/api/v1/users/${this.username}/repos`).then((res) => {
                return res;
            }).catch((err) => {
    ...
    async getBranchsList(projectId, repoName){
        ...
        //     }
        // }
        let response = await axios.get(`https://${this.endpoint}/api/v1/repos/${projectId}/${repoName}/branches`).then((res) => {
                return res;
            }).catch((err) => {
                if(err.response.data && err.response.data.includes("Invalid authorization token")){
                    _this.getToken();
    ...
    async getBooksList(userName,repoName, cloneId) {
        ...
        //     }
        // }
        let response = await axios.get(`https://${this.endpoint}/api/v1/repos/${userName}/${repoName}/git/trees/${cloneId}?recursive=true`).then((res) => {
                return res;
            }).catch((err) => {
                if(err.response.data && err.response.data.includes("Invalid authorization token")){
                    _this.getToken();
    ...
    async getDoorBookData(projectId, bookId, branchName, repo){
        ...
        return await axios.get(`https://${this.endpoint}/${projectId}/${repo}/raw/branch/${branchName}/${bookId}`).then((res) => {
            return res.data;
        }).catch((err) =>{
            if(err.response.data && err.response.data.includes("Invalid authorization token")){
    ...
    //exporting
    async updateDoorBookData(projectId, repo, contents, bookId, branch, attempt = 1){
        //convert in usx
        ...
        // }}
        return await axios.post(`https://${this.endpoint}/api/v1/repos/${projectId}/${repo}/${contents}/${bookId}?ref=${branch}`).then((res) => {
            return res.data;
        }).catch((err) =>{
            if(err.response.data && err.response.data.includes("Invalid authorization token")){
                if (attempt === 3) throw err;
                
                return this.updateBookData(projectId, repo, contents, bookId, branch, attempt + 1);
            }
