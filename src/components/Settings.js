...
class SettingsModal extends React.Component {
  ...
  saveJsonToDB = dir => {
    ...
    usfm_import
      .saveJsonToDb(dir, bibleName, refLangCodeValue, refVersion)
      .then(res => {
        res = mobx.toJS(AutographaStore.successFile);
        res.map(value => {
          this.setState(prevState => ({
  ...
  getSyncProvider = () => {
    return (
      (this.state.sync && this.state.sync.syncProvider) ||
      AutographaStore.syncProvider ||
      "paratext"
    );
  };
  ...
  listSyncProjects = async (
    ...
    this.props.showLoader(true);
    const syncAdapter = this.newSyncAdapter(
      syncProvider,
      username,
      password,
      endpoint
                  ...
                  <Tab.Content animation>
                    ...
                    <Tab.Pane eventKey="seventh">
                      ...
                      >
                        <Tabs
                          id="syncProvider"
                          onSelect={this.setSyncProvider}
                          defaultActiveKey={this.getSyncProvider()}
                        >
                          <Tab
                            eventKey="paratext"
                            title={`${
                              AutographaStore.currentTrans["label-paratext"]
                            }`}
                          >
                            {
                              <Ahref
                                i18n="text-info-paratext"
                                href="https://registry.paratext.org"
                              />
                            }
                            <PanelGroup
                              accordion
                              id="paratext-credential"
...
class CredentialPanel extends React.Component {
  ...
  render() {
    ...
    return (
      <Panel eventKey="creds">
        <Panel.Heading id="credential-title">
          <Panel.Title toggle onClick={onHeaderClick}>
